...
export class FoodLookupService {
  ...
  // §5.3 — generic/whole-food search via USDA. Each result is normalized
  // into the cache as it's returned (so a later diary-log referencing one of
  // these results resolves the same cache row rather than re-fetching).
